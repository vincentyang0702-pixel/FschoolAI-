...
-- ============================================================
-- APP TABLES — run these in Supabase SQL Editor
-- ============================================================
...
-- This app uses its own UUID-based identity (no Supabase Auth),
-- so auth.uid() is always null. Open policies are intentional.
alter table public.users       enable row level security;
alter table public.canvas_data enable row level security;

drop policy if exists "open_users"       on public.users;
drop policy if exists "open_canvas_data" on public.canvas_data;

create policy "open_users"       on public.users       for all using (true) with check (true);
create policy "open_canvas_data" on public.canvas_data for all using (true) with check (true);
